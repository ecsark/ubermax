    ...
    for t = 1:24
        for c = 1:clusterN
            for r = 1:clusterN
                ...
                if cluster_info(t,c,r,3) == 0 || cluster_info(t,c,r,1) == 0 || cluster_info(t,c,r,2) > 150 
...
for secStation = 1:clusterN
    [~,~,timeNeed] = GainT([startP,secStation,endP],sumOut,cluster_info,startTime);
    if timeNeed > DesireTime
        ...
        for nextP2  = 0:clusterN
            for nextP3 = 0:clusterN
                for nextP4 = 0:clusterN
                    for nextP5 = 0:clusterN
                        tripR = [startP,secStation,nextP2,nextP3,nextP4,nextP5];
                        [earnExp,earnM,earnT,lastTrip,~] = GainT(tripR,sumOut,cluster_info,startTime);
                        if lastTrip == endP && (DesireTime -earnT)>-DesireTime*time_buff
                            if maxEarn < earnM
                                maxEarn = earnM;
        ...
        [~,maxEarn_R_M(secStation),maxEarn_R_T(secStation),~,tripN] = GainST(maxRoutine,sumOut,cluster_info,startTime);


        NextStationREC(secStation,1:6) = maxRoutineE;
        [~,maxEarnE_R_M(secStation),maxEarnE_R_T(secStation),~,tripNE] = GainST(maxRoutineE,sumOut,cluster_info,startTime);
...
hold on 
for i = 1:5
    plot(Center(idxOPT(i),2),Center(idxOPT(i),1),'bx','MarkerSize',5,'LineWidth',2);
    hold on
    str = num2str(i);
...
for i = 1:5
    plot(Center(idxREC(i),2),Center(idxREC(i),1),'bx','MarkerSize',5,'LineWidth',2);
    hold on
    str = num2str(i);
    text(Center(idxREC(i),2),Center(idxREC(i),1),str,'FontSize', 20)
end
